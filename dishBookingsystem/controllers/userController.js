...
const signupUser = async (req, res) => {
  try {
    ...
    bcrypt.hash(password, saltRounds, async function (err, hash) {
       if (err) {
        return res
          .status(404)
          .json({
            Error: "Error in hasing the signup password",
            errr: err.messgae,
          });
      } else {
    ...
    res
      .status(404)
      .json({ Error: "Error in Signing up a user", errr: err.message });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    bcrypt.compare(password, hash, async function (err, result) {
      if (result == true) {
        let token = jwt.sign({ userId: user._id, role: user.role }, "praveena");
...
const resetPassword = async (req, res) => {
  try {
    let { token } = req.query;
    let decoded = jwt.verify(token, "praveena");
    if (decoded) {
